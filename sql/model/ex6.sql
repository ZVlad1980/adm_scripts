...
model
  ...
  (
    ...
    x0[iteration_number+1] = case 
      when sign(y(x[0])) = sign(y(x0[iteration_number])) then x[0]
      else x0[iteration_number]
    end,
    x1[iteration_number+1] = case
      when sign(y(x[0])) = sign(y(x1[iteration_number])) then x[0]
