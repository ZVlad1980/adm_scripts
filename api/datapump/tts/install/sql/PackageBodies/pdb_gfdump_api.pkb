create or replace package body gfdump_api is
  ...
  begin
    ...
    for i in 1..p_schemas.count loop
      ...
      ei('alter user ' || p_schemas(i).name || ' identified by ' || lower(p_schemas(i).name) || ' account unlock');
  ...
  begin
    ...
    if not p_continue then
      plog('Prepare schema start', true);
      --
      prepare_schemas(p_schemas, l_schemas);
      --
      plog('Prepare schema complete', true);
      --
